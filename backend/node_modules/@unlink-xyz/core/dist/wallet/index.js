...
function createIndexedDbStorage(opts = {}) {
  ...
  return {
    ...
    async put(key, value) {
      ...
      await wrapRequest(store.put(copy(value), key));
      await done;
    },
    async delete(key) {
      validateKey(key);
...
function beBufferToBigInt(value) {
  ...
  return BigInt(`0x${Buffer4.from(value).toString("hex")}`);
}
function leBufferToBigInt(value) {
  requireTypes(value, "value", ["Buffer", "Uint8Array"]);
  return BigInt(`0x${Buffer4.from(value).reverse().toString("hex")}`);
}
...
function leBigIntToBuffer(value, size) {
  ...
  const buffer = Buffer4.alloc(size, 0);
  const fromHex = Buffer4.from(hex, "hex").reverse();
...
function isBuffer2(value) {
  return Buffer5.isBuffer(value);
}
function requireBuffer(parameterValue, parameterName) {
  if (!isBuffer2(parameterValue)) {
...
function isBigNumberish2(value) {
  return isNumber2(value) || isBigInt2(value) || isStringifiedBigInt2(value) || isHexadecimal2(value) || isBuffer3(value) || isUint8Array2(value);
}
...
var IMT = class _IMT {
  ...
  constructor(hash2, depth, zeroValue, arity = 2, leaves = []) {
    ...
    requireTypes2(zeroValue, "zeroValue", ["number", "string", "bigint"]);
   ...
   */
  indexOf(leaf) {
    requireTypes2(leaf, "leaf", ["number", "string", "bigint"]);
  ...
  static verifyProof(proof, hash2) {
    ...
    requireTypes2(proof.root, "proof.root", ["number", "string", "bigint"]);
    requireTypes2(proof.leaf, "proof.leaf", ["number", "string", "bigint"]);
...
function encodeJson(value) {
  return encoder3.encode(JSON.stringify(value));
}
function decodeJson(payload) {
  try {
    return JSON.parse(decoder3.decode(payload));
  } catch {
